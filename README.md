...
Базовая скорость обучения (base_lr) обычно указывается для батча размером 64. Если используемый batch_size отличается, то максимальное значение (max_lr) масштабируется пропорционально:$$max\_lr = base\_lr \times \left(\frac{batch\_size}{64}\right)$$Также определяется min_lr, который составляет 5% от max_lr.Фаза Warmup (прогрев):В начале обучения (в течение warmup_epochs) скорость обучения постепенно увеличивается от нуля до max_lr. Это необходимо для стабилизации градиентов на ранних этапах:$$lr = \frac{max\_lr}{warmup\_epochs} \times (epoch + 1)$$ Фаза Cosine Annealing:
...
$$progress = \frac{epoch - \text{warmup\_epochs}}{\text{total\_epochs} - \text{warmup\_epochs}}$$ $$lr = \text{min\_lr} + 0.5 \times (\text{max\_lr} - \text{min\_lr}) \times (1.0 + \cos(\pi \times progress))$$
Управление аугментациями и L1 Loss: Согласно рекомендациям авторов YOLOX, за 15 эпох до конца обучения (no_aug_epochs) отключаются мозаичные аугментации и включается L1 Loss. 
